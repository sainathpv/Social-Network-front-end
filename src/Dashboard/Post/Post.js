...
class Post extends React.Component {
    ...
    try {
      fetch(
        ...
        .then(result => {
          ...
          fetch(
            'http://' + process.env.REACT_APP_API_HOST + '/posts/postComment',
            options
          )
            .then(result => {
              return result.json();
  ...
  votePost(vote) {
    ...
    fetch(
      'http://' + process.env.REACT_APP_API_HOST + '/posts/postvote',
      options
    )
      .then(result => {
        return result.json();
              ...
              <div>
                ...
                <ul className='d-inline'>
                  {this.state.tags.map((tag, i) => (
                    <li className='d-inline' key={i.toString()}>
                      {tag}
                    </li>
                  ))}
                </ul>
            ...
            <ul className='comments'>
              ...
              {this.state.comments.map((comment, i) => (
                <li key={i.toString()} className='comment bg-secondary'>
                  <button className="miniProfile" 
                    onClick={() =>
                      this.showMiniProfile(comment.profile, comment.user)
                    }
                  >
                    {comment.user}
                  </button>
  ...
  render() {
    return (
      <div className='post bg-primary border-lg border-round-small'>
        ...
        <button className="miniProfile" 
          onClick={() =>
            this.showMiniProfile(this.state.currentProfileID, this.state.name)
          }
        >
          {this.state.name}
        </button>
