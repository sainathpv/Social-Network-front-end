...
class PostForm extends Component {
  ...
  handleChange(event) {
    ...
    this.setState({
      content: document.getElementById('postFormContent').value
    });
  }
      ...
      .then(result => {
        ...
        if (this.state.type === 'poll') {
          var categories = Array.from(
            document.getElementById('postFormCategories').childNodes
        ...
        if (this.state.type === 'image') {
          ...
          fetch(
            'http://' + process.env.REACT_APP_API_HOST + '/posts/postPosts',
            options
          )
            .then(result => {
              result.json();
    ...
    if (this.state.type === 'text') {
      ...
      return (
        <div>
          ...
          <textarea
            ...
            style={style}
            id='postFormContent'
            className='w-100 text-primary border-round-small bg-secondary border-lg'
          ></textarea>
        </div>
      );
      ...
      return (
        <div>
          ...
          <input
            onChange={this.handleChange}
            id='postFormContent'
            className='w-100 text-primary border-round-small bg-secondary border-lg d-block'
            type='text'
            placeholder='Ex. https://www.youtube.com/watch?v=Tzl0ELY_TiM'
          ></input>
        </div>
      );
      ...
      return (
        <div>
          ...
          <ul id='postFormCategories' className='itemList'></ul>
          <br />
          <label>Category:</label>
          <input
            className='itemInput w-100 text-primary border-round-small bg-secondary border-lg d-block'
            type='text'
          ></input>
          <button onClick={this.addItem} className='btn-primary'>
            Add Category
          </button>
        </div>
      );
    ...
    return (
      <div className='postForm d-block m-auto bg-primary p-fixed border-lg border-round-small'>
        ...
        >
          ...
          <input
            id='postFormTitle'
            className='d-block text-primary border-round-small bg-secondary border-lg w-100'
          ></input>
          <label>Type Of Post:</label>
          <DropDownMenu
            items={['Video', 'Image', 'Text', 'Poll']}
            label='Text'
